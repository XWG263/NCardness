...
namespace NBC.ActionEditor
{
    ...
    public class AssetPlayer
    {
        ...
        void InternalSamplePointers(float currentTime, float previousTime)
        {
            ...
            var endCount = timePointers.Count(_=>_.end);
            if (endCount * 2 == timePointers.Count)
            {
        ...
        public void InitializePreviewPointers()
        {
            ...
            var childs = EditorTools.GetTypeMetaDerivedFrom(typeof(ProcessorBase));
            ...
            foreach (var group in Asset.groups.AsEnumerable().Reverse())
            {
                ...
                foreach (var track in group.Tracks.AsEnumerable().Reverse())
                {
                    ...
                    {
                        if (Activator.CreateInstance(t1) is ProcessorBase preview)
                        {
                    ...
                    foreach (var clip in track.Clips)
                    {
                        ...
                        {
                            if (Activator.CreateInstance(t) is ProcessorBase preview)
                            {
