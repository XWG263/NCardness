...
namespace CCx
{
    public class CardVisualItem : MonoBehaviour
    {
        ...
        public void Initialize(CardVisual target, int index = 0)
        {
            //Declarations
            parentCard = target;
        ...
        private void HandPositioning()
        {
            ...
            var tt = parentCard.NormalizedPosition();
            curveRotationOffset = curve.rotation.Evaluate(tt);
        }
        ...
        private void Select(CardVisual card, bool state)
        {
            DOTween.Kill(2, true);
            float dir = state ? 1 : 0;
        ...
        private void EndDrag(CardVisual card)
        {
            canvas.overrideSorting = false;
            transform.DOScale(1, scaleTransition).SetEase(scaleEase);
        }

        private void PointerEnter(CardVisual card)
        {
            if (scaleAnimations)
                transform.DOScale(scaleOnHover, scaleTransition).SetEase(scaleEase);
        ...
        private void PointerExit(CardVisual card)
        {
            if (!parentCard.wasDragged)
                transform.DOScale(1, scaleTransition).SetEase(scaleEase);
        }

        private void PointerUp(CardVisual card, bool longPress)
        {
            if (scaleAnimations)
        ...
        private void PointerDown(CardVisual card)
        {
            if (scaleAnimations)
                transform.DOScale(scaleOnSelect, scaleTransition).SetEase(scaleEase);
